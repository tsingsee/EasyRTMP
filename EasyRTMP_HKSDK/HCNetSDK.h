...
typedef struct 
{
    ...
    33-2448*800,34-XGA（1024*768），35-SXGA（1280*1024）,36-WD1(960*576/960*480),37-1080i,38-WSXGA(1440*900)
	39-HD_F(1920*1080/1280*720),40-HD_H(1920*540/1280*360),41-HD_Q(960*540/630*360)
	42-2336*1744, 43-1920*1456, 44-2592*2048, 45-3296*2472, 46-1376*768, 47-1366*768,48-1360*768,49-WSXGA+,56-2304*1296,57-WXGA(1280*800)
	0xff-Auto(使用当前码流分辨率)*/
    BYTE byResolution;  	
...
{	
    ...
    /*0x200: 抓图并上传FTP*/ 
    DWORD   dwMaxRelAlarmOutChanNum ; //触发的报警输出通道数（只读）最大支持数量
    DWORD   dwRelAlarmOut[MAX_ALARMOUT_V40]; //实际触发的报警输出号，按值表示,采用紧凑型排列，从下标0 - dwRelAlarmOutChanNum -1有效，如果中间遇到0xffffffff,则后续无效
	NET_DVR_SCHEDTIME struAlarmTime[MAX_DAYS][MAX_TIMESEGMENT_V30]; /*布防时间*/
 /*触发的录像通道*/
...
typedef struct
{
    ...
    /*0x200: 抓图并上传FTP*/ 
    DWORD   dwMaxRelAlarmOutChanNum ; //触发的报警输出通道数（只读）最大支持数量
	DWORD   dwRelAlarmOut[MAX_ALARMOUT_V40]; /*触发报警输出号，按值表示,采用紧凑型排列，从下标0 - dwRelAlarmOutChanNum -1有效，如果中间遇到0xffffffff,则后续无效*/  
NET_DVR_SCHEDTIME struAlarmTime[MAX_DAYS][MAX_TIMESEGMENT_V30]; /*布防时间*/
BYTE  byRes[64]; //保留
...
typedef struct tagNET_DVR_CAMERA_SETUPCFG
{
	...
	DWORD dwPitchAngle;      //相机俯仰角度[-180°, 180°],  浮点数 * 1000
	DWORD dwInclineAngle;     //相机倾斜角度[-180°,180°]，浮点数 * 1000
	DWORD dwRotateAngle;     //相机旋转角度[-180°,180°]，浮点数 * 1000

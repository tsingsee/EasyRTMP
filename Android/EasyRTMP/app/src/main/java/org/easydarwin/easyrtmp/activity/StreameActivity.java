...
import org.easydarwin.easyrtmp.updatemgr.UpdateMgr;
...
public class StreameActivity extends AppCompatActivity implements SurfaceHolder.Callback, View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP){
            btnPushScreen.setVisibility(View.GONE);
        }

        if (RecordService.mEasyRTMP != null){
    ...
    private void startScreenPushIntent() {
        if (StreameActivity.mResultIntent != null && StreameActivity.mResultCode != 0) {
            ...
            RecordService.mEasyRTMP = new EasyRTMP();
        ...
        switch (v.getId()) {
            ...
            case R.id.btn_switch:
                ...
                if (!mMediaStream.isStreaming()) {
                        ...
                        public void onCallback(int code) {
                            switch (code) {
                                ...
                                case CODE.EASY_ACTIVATE_SUCCESS:
                                    sendMessage("激活成功");
                                    break;
                                case CODE.EASY_RTMP_STATE_CONNECTING:
                                    sendMessage("连接中");
                                    break;
                                case CODE.EASY_RTMP_STATE_CONNECTED:
                                    sendMessage("连接成功");
                                    break;
                                case CODE.EASY_RTMP_STATE_CONNECT_FAILED:
                                    sendMessage("连接失败");
                                    break;
                                case CODE.EASY_RTMP_STATE_CONNECT_ABORT:
                                    sendMessage("连接异常中断");
                                    break;
                                case CODE.EASY_RTMP_STATE_PUSHING:
                                    sendMessage("推流中");
                                    break;
                                case CODE.EASY_RTMP_STATE_DISCONNECTED:
                                    sendMessage("断开连接");
                                    break;
                    ...
                    txtStreamAddress.setText(url);
                    spnResolution.setEnabled(false);
                } else {
                    mMediaStream.stopStream();
                    btnSwitch.setText("推送摄像头");
                    spnResolution.setEnabled(true);
